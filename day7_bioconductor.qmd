...
# Kevin used: something using sum, is.na etc


```{r}
length(unique(na.omit(gtf_ACTB$transcript_id)))
...
```{r}
human_symbols <- keys(x = org.Hs.eg.db, "SYMBOL"  ) [1:6]
...
> Answer: This causes issues, as 'select' in tidyverse behaves differently. Depending on the order you load packages the behaviour of the command will change. 
> 
