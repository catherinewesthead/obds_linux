...
> Answer: Data is mostly numerical with some character formats. There are 150 rows and 5 columns in this tibble. 
...
> Answer: No, the columns and data are misaligned. I am not sure how to rectify this. 
...
summarise(
  ...
  Sepal.Length.min = min(sepal_length),
  Sepal.Length.max = max(sepal_length))
...
labs (title = "Sepal Area of the Iris Dataset", x = "Sepal Area", y = "Frequency") +
scale_fill_brewer(palette = "Set3")


```
## Make a facet plot by species 
...
## First I have added row_id column, to ensure I can identify all measurements
...
## I have then made iris_long including the row_id so all of the measurements are still assigned to specific flowers
```{r}
iris_long <- iris_wide %>%  
  pivot_longer(
  where(is.numeric),
...
iris_long2 <- iris_wide %>%  
  pivot_longer(
  cols = c(where(is.numeric), -row_id),
  names_to = "feature", 
  values_to = "measurement")
