...
```
## Managing aesthetics of my plot
```{r}
help(barplot)
barplot(explained_variance_ratio, 
        xlab = "PC",
...
# Understanding teh cutree function 
```{r}
help(cutree)
```
...
```
# Experimenting with plotting cutree
```{r}
plot(iris_hclust_ward_labels)
...
par(mfrow = c(2, 2))
plot(iris_hclust_ward_labels )
plot(iris_hclust_complete_labels )
plot(iris_hclust_average_labels )
plot(iris_hclust_single_labels)
par(mfrow = c(1, 1))
```
## Plot lenght vs width with clusters overlayed. DId this for different methods to see the different visualisations. 
```{r}
help(plot)
plot(iris$Sepal.Length, iris$Sepal.Width,
     ...
     xlab = "Sepal Length",
     ylab = "Sepal Width",
     main = "Iris Original Data Set - Length vs Width",
     col =iris_hclust_average_labels,
     pch = 19
     )
```
```{r}
